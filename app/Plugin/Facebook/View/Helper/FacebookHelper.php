	...
	public function login($options = array(), $label = '') {
		$options = array_merge(
			array(
				...
				'custom' => false,		// Se false, ignora todos campos exceto ID
				'redirect' => false,	// array() usado com Router::url
				'img' => false,			// nome da imagem usadada como botão (path: /Facebook/img/)
				'alt' => '',			// alt para imagem do botão
				'id' => '',				// #id HTML para o link
				'class' => '',			// class HTML para o link
	...
	public function logout($options = array(), $label = '') {
		...
		if ((isset($options['redirect']) && $options['redirect']) || $options['custom']) {
			...
			if ($options['img']) {
				...
				return $this->Html->link($options['label'], '#', array(
					'onclick' => $onclick, 'id' => $options['id'], 'class' => $options['class'], 'escape' => $options['escape'])
				);
			}
		} else {
