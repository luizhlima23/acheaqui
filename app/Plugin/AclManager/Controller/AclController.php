...
class AclController extends AclManagerAppController {
	...
	public function drop() {
		...
		$this->Session->setFlash(__d('acl_manager', "Both ACOs and AROs have been dropped"), 'layout/flash/flash_info');
	...
	public function drop_perms() {
		if ($this->Acl->Aro->Permission->deleteAll(array("1 = 1"))) {
			$this->Session->setFlash(__d('acl_manager', "Permissions dropped"), 'layout/flash/flash_info');
		} else {
			$this->Session->setFlash(__d('acl_manager', "Error while trying to drop permissions"), 'layout/flash/flash_info');
		}
		$this->redirect(array("action" => "index"));
	}
	...
	public function permissions() {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			$this->Session->setFlash(__d('acl_manager', "The data were saved"), 'layout/flash/flash_info');
		}
	...
	public function update_acos() {
		...
		$this->Session->setFlash(sprintf(__d('acl_manager', "%d ACOs have been created/updated"), $count), 'layout/flash/flash_info');
	...
	public function update_aros() {
		...
		$this->Session->setFlash(sprintf(__d('acl_manager', "%d AROs have been created"), $count), 'layout/flash/flash_info');
		$this->redirect($this->request->referer());
	}
	...
	protected function _getAuthorizer() {
		...
		if (empty($this->_authorizer)) {
			$this->Session->setFlash(__d('acl_manager', "ActionAuthorizer could not be found"), 'layout/flash/flash_info');
