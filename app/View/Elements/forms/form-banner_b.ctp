<?php
	$id = $this->request->data['BannerB']['id'];

	# Imagem
	$url_imagem = $this->Upload->url($this->request->data, 'BannerB.imagem', array('style'=>'original'));
	$posicao = strpos($url_imagem, $this->webroot);
	$file = substr($url_imagem, $posicao+strlen($this->webroot)); 
?>
<div class="role form">
	
	<?php echo $this->Form->create('BannerB', array('role' => 'form', 'type'=>'file')); ?>
 				...
 				<div class="col-md-12">
					<?php 
						...
						echo $this->Form->postLink($icon_trash, 
							array(
								'controller'=>'banners',
								'action'=>'delete_banner_rotativo_img',
								$id
							),
							array(
								'inline'=>false,
