<?php
	$id = $this->request->data['BannerC']['id'];
	
	# Imagem
	$url_imagem = $this->Upload->url($this->request->data, 'BannerC.imagem', array('style'=>'original'));
	$posicao = strpos($url_imagem, $this->webroot);
	$file = substr($url_imagem, $posicao+strlen($this->webroot)); 
?>
<div class="role form">
	
	<?php echo $this->Form->create('BannerC', array('role' => 'form', 'type'=>'file')); ?>
 				...
 				<div class="col-md-12">
					<?php 
						...
						echo $this->Form->postLink($icon_trash, 
							array(
								'controller'=>'banners',
								'action'=>'delete_banner_premium_img',
								$id
							),
							array(
								'inline'=>false,
